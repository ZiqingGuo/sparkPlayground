...
rawText = sc.textFile(file_path).cache()
print_count(rawText)
...
def safe_parse(raw_json):
    ...
    try:
        ...
        if ('created_at' in json_object) and ('text' in json_object) and ('user' in json_object):
            return json_object
        else:
            return None
...
validJson = rawText.map(safe_parse).filter(lambda x: x is not None)
validTweet = validJson.map(lambda x: (x['user']['id_str'],x['text'])).cache()
...
timer_start()
usersCount = validTweet.map(lambda x:x[0]).distinct().count()
print_users_count(usersCount)
...
timer_start()
tweetPartition = validTweet.map(lambda x:(partition[x[0]] if x[0] in partition else 7,1))
partitionCount = tweetPartition.reduceByKey(lambda x,y:x+y).sortByKey()
timer_start()
...
print_post_count(partitionCount.collect())
...
timer_start()
tokensRdd = validTweet.flatMapValues(tok.tokenize).distinct().cache()
tokensCount = tokensRdd.map(lambda x:x[1]).distinct()
timer_stop("Tokenize")
print_count(tokensCount)
...
timer_start()
tokensUser = tokensRdd.map(lambda x: (x[1],1))
tokensFiltered = tokensUser.reduceByKey(lambda x,y:x+y).filter(lambda x:x[1]>=100)
tokensOverall = tokensFiltered.sortBy(lambda x:(x[1],x[0]),False).cache()
print_count(tokensOverall)
print_tokens(tokensOverall.take(20))
...
timer_start()
tokensGroup = tokensRdd.map(lambda x:(x[1],x[0]))    .join(tokensOverall)    .map(lambda x:((partition[x[1][0]] if x[1][0] in partition else 7,x[0],x[1][1]),1))    .reduceByKey(lambda x,y:x+y)    .map(lambda x:(x[0][0],x[0][1],get_rel_popularity(x[1],x[0][2])))    .cache()
#(g,t,p)
for groupId in range(7+1):
    tokensByGroup = tokensGroup.filter(lambda x:x[0]==groupId)        .map(lambda x:((-x[2],x[1]),x[1]))        .sortByKey()        .map(lambda x:(x[1],-x[0][0]))
    #l = tokensByGroup.glom().map(len).collect()
    #print(l)
    print_tokens(tokensByGroup.take(10),groupId)
